...
#include <aspect/initial_conditions/S40RTS_perturbation_Gurnis.h>
...
namespace aspect
{
  namespace InitialConditions
  {
    ...
    template <int dim>
    void
    S40RTSPerturbation_Gurnis<dim>::initialize()
    {
    ...
    initial_temperature (const Point<dim> &position) const
    {
     ...
     double perturbation = s(scoord[0]);
     ...
     const double depth = this->get_geometry_model().depth(position);
      ...
      double dens_scaling = vs_to_density;
      // Perturbations in upper 660km are always zerod out.
       if (depth <= 660000)
         dens_scaling = 0;

       if (vs_to_density_S4 == true)
       {
         if (depth <= 660000)
          dens_scaling = 0;
     ...
     const double temperature = reference_temperature + temperature_perturbation;
    ...
    template <int dim>
    void
    S40RTSPerturbation_Gurnis<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Initial conditions");
      {
        prm.enter_subsection("S40RTS perturbation Gurnis");
        {
    ...
    template <int dim>
    void
    S40RTSPerturbation_Gurnis<dim>::parse_parameters (ParameterHandler &prm)
    {
...
namespace aspect
{
  namespace InitialConditions
  {
    ASPECT_REGISTER_INITIAL_CONDITIONS(S40RTSPerturbation_Gurnis,
                                       "S40RTS perturbation Gurnis",
