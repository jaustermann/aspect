...
#include <aspect/initial_temperature/Gypsum.h>
...
namespace aspect
{
  namespace InitialTemperature
  {
    ...
    namespace internal
    {
      ...
      {
        public:
          GypsumLookup(const std::string &filename)
          {
            std::string temp;
          ...
          const std::vector<double> &shear_values() const
          {
            return coeffs;
          }
    ...
    template <int dim>
    void
    GypsumPerturbation<dim>::initialize()
    {
      gypsum_lookup.reset(new internal::GypsumLookup(datadirectory+gypsum_file_name));
    ...
    initial_temperature (const Point<dim> &position) const
    {
      ...
      double depth_values[] = {0, 100, 175, 250, 325, 400, 525, 650, 750, 850, 1000, 1150, 1300, 1450, 1600, 1750, 1900, 2050, 2200, 2350, 2500, 2650, 2900};
      ...
      double shear_pert[22][65341];

      int next_ind = 0;
      for (int i=0; i<65341; i++)
        for (int j=0; j<22; j++)
          {
            shear_pert[j][i] = coeffs[next_ind];
            next_ind += 1;
          }
...
// start geotherm stuff
    ...
    template <int dim>
    void
    GypsumPerturbation<dim>::declare_parameters (ParameterHandler &prm)
    {
    ...
    template <int dim>
    void
    GypsumPerturbation<dim>::parse_parameters (ParameterHandler &prm)
    {
      ...
      prm.enter_subsection("Initial temperature model");
      {
        prm.enter_subsection("Gypsum perturbation");
        {
          ...
          gypsum_file_name = prm.get ("Initial condition file name");
...
namespace aspect
{
  namespace InitialTemperature
  {
    ASPECT_REGISTER_INITIAL_TEMPERATURE_MODEL(GypsumPerturbation,
                                              "Gypsum perturbation",
