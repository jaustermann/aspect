...
#include <aspect/material_model/glisovic_forte_old.h>
...
namespace aspect
{
  namespace MaterialModel
  {
    ...
    template <int dim>
    void
    GlisovicForteOld<dim>::initialize()
    {
...
//   }
               ...
               const Point<dim> &position) const
    {
      ...
      const double vis_lateral_exp = - thermal_viscosity_exponent * delta_temperature;
...
//      if(depth > (6371000 - 6346600))
                                   ...
                                   const Point<dim> &position) const
    {
      ...
      const double depth = this->get_geometry_model().depth(position);
      double thermal_alpha_val = thermal_alpha; // set constant as default
      double B_val, A_val;
    ...
    template <int dim>
    void
    GlisovicForteOld<dim>::declare_parameters (ParameterHandler &prm)
    {
...
//                             "purpose of simulating backward advection.");
    ...
    template <int dim>
    void
    GlisovicForteOld<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Glisovic Forte Old model");
        {
          ...
          thermal_viscosity_exponent   = prm.get_double ("Temperature to viscosity scaling");
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(GlisovicForteOld,
                                   "Glisovic Forte Old",
