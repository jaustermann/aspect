...
#include <aspect/material_model/gurnis_lat.h>
  ...
  { 
               ...
               const Point<dim> &position) const
    {
     ...
     const double dvs = s(scoord[0]);
      ...
      // Set lateral variations to 0 in uppermost X km. Based on T model
      if (depth <= depth_zero_lat)
        vis_lateral_exp = 1.0/eta_star;

      // Set lateral variations to 0 outside regional region
     double theta = scoord[2] * 180/numbers::PI;
     theta -= 90.;
     theta *= -1.;

     const double phi = scoord[1] * 180/numbers::PI;

     if (perturbations_regional == true)
       if (phi >= 70 && phi <= (360-30) && (theta >= 50 || theta <= -50))
          vis_lateral_exp = 1.0/eta_star;
   ...
   Gurnis_lat<dim>::
    ...
    density (const double temperature,
             ...
             const Point<dim> &position) const
    {
       const double rho = reference_rho * (1 - thermal_alpha * (temperature - this->get_adiabatic_conditions().temperature(position)));
      return rho;
    }
    ...
    template <int dim>
    void
    Gurnis_lat<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Gurnis lat model");
        {
          ...
          prm.declare_entry ("Initial condition file name", "S20RTS.sph",
                             Patterns::Anything(),
          ...
          prm.declare_entry ("Depth at which to zero out lat visc","200000.0",
                             Patterns::Double (0),
                             "");
          prm.declare_entry ("Apply perturbations only regional", "false",
                             Patterns::Bool(),
                             "");
        }
    ...
    template <int dim>
    void
    Gurnis_lat<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Gurnis lat model");
        {
          ...
          depth_zero_lat               = prm.get_double ("Depth at which to zero out lat visc");
          perturbations_regional  = prm.get_bool("Apply perturbations only regional");
        }
        prm.leave_subsection();
      }
      prm.leave_subsection();
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(Gurnis_lat,
                                   "Gurnis lat",
